...
namespace TDDSample.Tests.UnitTests.Shared;
...
public class UserHttpClientTests : IClassFixture<MappingFixture>
{
    ...
    public async Task get_all_users_should_return_users_list()
    {
        ...
        var expectedPageList = new PagedList<User>
        {
            Results = users,
            PageSize = pageSize,
            PageNumber = page,
        ...
        result
            .Should()
            .BeEquivalentTo(expectedPageList, c => c.ExcludingMissingMembers().Excluding(s => s.TotalNumberOfPages));
    }
