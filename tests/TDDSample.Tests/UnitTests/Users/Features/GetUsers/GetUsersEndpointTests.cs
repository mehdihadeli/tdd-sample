...
public class GetUsersEndpointTests
{
    ...
    public async Task handle_should_return_ok_status_code()
    {
        ...
        var pageResult = new PagedList<UserDto>
        {
            PageSize = pageSize,
            PageNumber = page,
            Results = users
        ...
        mediator
            .Send(
                Arg.Is<TDDSample.Users.GetUsers.GetUsers>(x =>
                    x.PageRequest.Page == page && x.PageRequest.PageSize == pageSize
                ),
    ...
    public async Task handle_with_http_response_exception_should_returns_http_problem()
    {
        ...
        mediator
            .Send(Arg.Any<TDDSample.Users.GetUsers.GetUsers>(), cancellationToken)
