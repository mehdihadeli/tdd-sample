...
public record UpdateTodoItem(int Id, string Title, bool IsCompleted, int UserId)
    : IRequest<OneOf<None, BadRequestException, NotFoundException>>;

internal class UpdateTodoItemHandler
    : IRequestHandler<UpdateTodoItem, OneOf<None, BadRequestException, NotFoundException>>
{
    ...
    )
    {
        ...
        if (todoItem is null)
            return new NotFoundException($"TodoItem with id '{request.Id}' not found");
